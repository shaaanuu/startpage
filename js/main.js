...
function initSearchBar(jsonData) {
  ...
  document.getElementById(
    searchBarId
  ).placeholder = `Search something on ${searchEngine}`;
  document.getElementById(searchBarId).addEventListener("keypress", (event) => {
    ...
    if (query == "--setting" || query == "--settings") {
      ...
      document.getElementById(
        "search-bar-input-autocomplete-list"
      ).style.display = "none";
      return;
    }
...
function buildMsg() {
  ...
  if (inRange(currentTime, 0, 5.59)) return "It's too late, take some sleep";
  if (inRange(currentTime, 6, 8.59)) return "You're up early";
  if (inRange(currentTime, 9, 11.59)) return "Have a good day ahead";
  if (inRange(currentTime, 12, 16.59)) return "Good Afternoon";
  if (inRange(currentTime, 17, 19.59)) return "Good Evening";
  if (inRange(currentTime, 20, 24)) return "It's time to wrap up for the day";
  else return "";
}
...
function handleMessage(userName) {
  ...
  builtMsg == ""
    ? (builtMsg = `Hello ${userName}`)
    : (builtMsg = `Hey ${userName}, ${builtMsg}!`);
  return builtMsg;
}
...
function parseAndCreate(jsonData) {
  ...
  if (
    Object.keys(jsonData).includes("message") &&
    typeof jsonData["message"] == "string" &&
    jsonData["message"] != ""
  )
    builtMsg = jsonData["message"];
  else builtMsg = this.handleMessage(this.userName);
  ...
  if (jsonData["style"]) {
    ...
    //   document.getElementById(weatherId).style.color =
    //     styleData["weatherColor"];
    // }
    if (styleData["searchColor"]) {
      document.getElementById(searchBarId).style.color =
        styleData["searchColor"];
    }
    if (styleData["searchBackgroundColor"]) {
      document.getElementById(searchBarId).style.backgroundColor =
        styleData["searchBackgroundColor"];
    ...
    if (styleData["searchPlaceholderColor"]) {
      document
        ...
        .classList.add(
          createPlaceholderStyleClass(styleData["searchPlaceholderColor"])
        );
    }
    if (styleData["autocompleteHighlightBackgroundColor"]) {
      addAutocompleteStyleClass(
        styleData["autocompleteHighlightBackgroundColor"]
      );
    }
...
function createSqr(sqrData, index, globalNewTabEnabled) {
  ...
  if (colorValid) customClass = createClass(color);
  else if (index > bgClassContainer.length - 1) customClass = "media";
  else customClass = bgClassContainer[index];
...
function extractQuickLinks(passedSqrs, style) {
  ...
  passedSqrs.forEach((linksContainer) => {
    linksContainer.links.forEach(
      (linkObject) => (this.validQuickLinks[linkObject.name] = linkObject.url)
    );
  });
...
function listenForSettings() {
  document.onkeyup = (event) => {
    ...
    if (event.ctrlKey && event.which == 188) showSettings();
  };
}
